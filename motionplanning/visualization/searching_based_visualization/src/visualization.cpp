...
void drawBlinkyCursor(sf::RenderWindow& window_,
    ...
    // Toggle cursor visibility every 0.5 seconds for blinking effect
    ...
    // Create and position the text with left alignment and vertical centering
    ...
    // Draw the blinking cursor at the end of the text
    if (cursorVisible_) {
...
void drawAlgorithmDropdownList(sf::RenderWindow& window_,
    ...
    // Calculate expanded height to accommodate all algorithm options
    int newHeight = height * (algorithms.size() + 1); // +1 for the default algorithm box
    ...
    // Detect if click is outside the dropdown area
    clickedOutside_ = (mousePos_x >= x && mousePos_x <= x + width &&
                       mousePos_y >= y && mousePos_y <= y + newHeight) ? false : true;
    
    // Render each algorithm option as a selectable item
    for (int i = 0; i < algorithms.size(); i++) {  
        ...
        // Render algorithm name centered vertically
        ...
        // Store the selected algorithm when hovered item is clicked
        if (isHovered) {
...
Visualizer::Visualizer(Grid& grid, Obstacle& obstacle) 
    ...
    // Load font once using static variable to avoid reloading
    ...
    // Create and configure the SFML window
    window_.create(sf::VideoMode(sf::Vector2u(windowWidth_, windowHeight_)), "Motion Planning Visualizer");
    window_.setFramerateLimit(60);
    
    // Calculate control panel position (to the right of grid)
    boxAlignmentX_ = gridOffsetX_ + gridWidth_ + gridOffsetX_ + 20;
    boxAlignmentY_ = 20;

    // Initialize all UI buttons with their positions and properties
...
void Visualizer::displayWindows() {
    ...
    for (int y = 0; y < grid_.getHeight(); ++y) {
        for (int x = 0; x < grid_.getWidth(); ++x) {
            ...
            gridCell.setOutlineThickness(2);
...
void Visualizer::handlePathPlanningBox() {
    /*
    Sequence:
    1. Init, show first algorithm in the list as default
    2. If user click on that, clear label and show drop-down list
    2.1. If user doesn't select anything and click outside the drop-down list => Set back to init algorithm
...
void Visualizer::resetWindows() {
    ...
    grid_.gridResize(grid_.getWidth(), grid_.getHeight()); // Reset to default size
    ...
    selectedAlgo_ = algorithms[0]; // Reset to default algorithm
